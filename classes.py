...
class Card(object):
    ...
    def power(self, other_card):
        ...
        50.0
        """
        total = (self.attack) - (other_card.defense / 2)
        return total
...
class Player(object):
    ...
    def play(self, card_index):
        ...
        2
        """
        playing_card = self.hand[card_index]
...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        True
        """
        # other_card = Card object
        # Assume opponent has at least 3 cards in hand AND deck
        for i in range(3):
            opponent.hand.pop(0)
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        # Add opp's attack/defense to all of player's cards
        for card in player.deck.cards:
            card.attack += other_card.attack
            card.defense += other_card.defense

        # Remove all cards in opp's deck that share attack/def with opponent's card
        # Require making a copy using [:] bc. iterating while modifying has issues
